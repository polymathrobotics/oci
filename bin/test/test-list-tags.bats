...
@test "default tag is nothing" {
  ...
  run list-tags.sh
  popd
  assert_output 'docker.io/polymathrobotics/image-no-toml'
}
...
@test "reads tags from toml" {
  ...
  run list-tags.sh
  popd
  assert_output --partial 'docker.io/polymathrobotics/example:1.2.3'
  assert_output --partial 'docker.io/polymathrobotics/example:latest'
}
...
@test "reads tags from toml with csv" {
  ...
  run list-tags.sh -c
  popd
  assert_output '"docker.io/polymathrobotics/example:1.2.3","docker.io/polymathrobotics/example:latest"'
}
...
@test "uses default tag when toml has name and platform" {
  ...
  run list-tags.sh
  popd
  assert_output 'docker.io/polymathrobotics/example'
}
