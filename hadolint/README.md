# hadolint [![hadolint](https://github.com/polymathrobotics/oci/actions/workflows/hadolint-push.yml/badge.svg)](https://github.com/polymathrobotics/oci/actions/workflows/hadolint-push.yml)

https://github.com/polymathrobotics/oci/tree/main/hadolint

Hadolint is a linter for Dockerfiles/Containerfiles. It helps you build [best practice](https://docs.docker.com/develop/develop-images/dockerfile_best-practices/) container images. 

The linter parses the Dockerfile into an AST and performs rules on top of the AST. It uses ShellCheck to lint the Bash code inside RUN instructions.

## How to use hadolint

Just pipe your `Dockerfile` or `Containerfile` to `docker run`:

```bash
docker container run --rm --interactive --tty \
    docker.io/polymathrobotics/hadolint < Dockerfile
# OR
docker container run --rm --interactive --tty \
    docker.io/polymathrobotics/hadolint < Containerfile
```

or using [Podman](https://podman.io/):

```bash
podman run --rm -i docker.io/polymathrobotics/hadolint < Dockerfile
# OR
podman run --rm -i docker.io/polymathrobotics/hadolint < Containerfile
```
...
dockerfile="$1"
shift
docker run --rm -i docker.io/polymathrobotics/hadolint hadolint "$@" - < "$dockerfile"
```
